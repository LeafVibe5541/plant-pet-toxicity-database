# Database di Tossicità delle Piante per Animali Domestici

> **Un database open source completo sulle piante da interno tossiche per gli animali domestici, con valutazioni di tossicità a livello di organo, casi di misdiagnosi e protocolli di cura adattivi all'ambiente.**

## Lingue

Lingua predefinita: **Inglese** (`README.md`)

[English](README.md) | [Français](README.fr.md) | [Deutsch](README.de.md) | [Italiano](README.it.md) | [Español](README.es.md) | [Português](README.pt.md) | [日本語](README.ja.md) | [한국어](README.ko.md) | [简体中文](README.zh-CN.md) | [繁體中文](README.zh-TW.md)

> Questo file è una traduzione della documentazione principale. La versione inglese resta la fonte più aggiornata.

## Panoramica

Molti strumenti di identificazione classificano la tossicità per animali solo come "tossica/non tossica". È un modello troppo semplice.

Questo database fornisce dati dettagliati:
- Per organo della pianta (foglia, fusto, polline, linfa, ecc.)
- In base a dose e taglia dell'animale
- In base al meccanismo tossico
- In base al contesto reale di esposizione

## Documenti principali

### Report di tossicità per animali

- [Aloe Vera Anthraquinone Risk](pet-toxicity/aloe-vera-anthraquinone-risk.md)
- [Azalea Grayanotoxin Cardiac Risk](pet-toxicity/azalea-grayanotoxin-cardiac-risk.md)
- [Lily Cat Toxicity](pet-toxicity/lily-cat-toxicity.md)
- [Pothos Hidden Toxicity](pet-toxicity/pothos-hidden-toxicity.md)

### Casi di misdiagnosi

- [Fungal Leaf Spot vs Sunburn](misdiagnosis/fungal-leaf-spot-vs-sunburn.md)
- [Spider Mites vs Dust](misdiagnosis/spider-mites-vs-dust.md)
- [Root Rot vs Underwatering](misdiagnosis/root-rot-vs-underwatering.md)
- [Scale Insects vs Mineral Residue](misdiagnosis/scale-insects-vs-mineral-residue.md)

### Protocolli di cura dinamici

- [Fiddle-Leaf Fig Moist Soil Myth](care-protocols/fiddle-leaf-fig-moist-soil-myth.md)
- [Monstera Watering Myth](care-protocols/monstera-watering-myth.md)


<!-- BEGIN AUTO-GENERATED-INDEX-REF -->
## Indice Completo degli Articoli

Questo README tradotto mantiene una panoramica compatta. L'indice completo generato automaticamente è mantenuto nel documento principale.

- Report di tossicità per animali：216
- Casi di diagnosi errata：190
- Protocolli di cura dinamica：164
- Indice completo: [README.md#full-generated-article-index](README.md#full-generated-article-index)

<!-- END AUTO-GENERATED-INDEX-REF -->

## Concetti chiave

- **PAWS (Pet-Aware Warning System)**: valutazione multidimensionale della tossicità, per organo e adattata alla taglia dell'animale
- **PF-BRI (PlantFun Bio-Rhythm Index)**: raccomandazioni dinamiche di irrigazione basate sul VPD (deficit di pressione di vapore)
- **Micro-distance Pathological Algorithm**: distinzione precoce tra stress biotico e abiotico

## Metodologia dei dati

1. Risoluzione a livello di organo
2. Documentazione dei meccanismi tossici
3. Specificità gatto/cane
4. Benchmark basati su evidenze
5. Aggiornamento trimestrale

## Contributi

I contributi sono benvenuti. Consulta [CONTRIBUTING.md](CONTRIBUTING.md).

## Citazione

```bibtex
@misc{plantfun_pet_toxicity_db_2026,
  title={Plant Pet-Toxicity Database: Organ-Level Toxicity Assessments for Household Plants},
  author={PlantFun Plant Pathology Lab},
  year={2026},
  publisher={GitHub},
  url={https://github.com/LeafVibe5541/plant-pet-toxicity-database}
}
```

## Licenza

Questo progetto è rilasciato con [CC0 1.0 Universal](LICENSE).
